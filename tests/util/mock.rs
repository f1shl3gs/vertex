...
use vertex::config::{
    DataType, HealthCheck, Output, SinkConfig, SinkContext, SourceConfig, SourceContext,
    TransformConfig, TransformContext,
};
use vertex::pipeline::{Pipeline, ReceiverStream};
use vertex::sinks::{Sink, StreamSink};
use vertex::sources::Source;
use vertex::transforms::{FunctionTransform, Transform};
...
impl SourceConfig for MockSourceConfig {
    async fn build(&self, cx: SourceContext) -> vertex::Result<Source> {
...
impl TransformConfig for MockTransformConfig {
    async fn build(&self, ctx: &TransformContext) -> vertex::Result<Transform> {
...
impl SinkConfig for MockSinkConfig {
    async fn build(&self, cx: SinkContext) -> vertex::Result<(Sink, HealthCheck)> {
        let sink = MockSink {
            acker: cx.acker(),
        ...
        Ok((
            vertex::sinks::Sink::Stream(Box::new(sink)),
            healthcheck.boxed(),
        ))
    }
