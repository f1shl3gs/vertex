...
impl Decoder {
    pub fn new(encoding: &'static Encoding) -> Self {
        Self {
            ...
            output: BytesMut::new(),
            // We explicitly choose not to remove BOM as part of encoding_rs's decoding
            // capabilities: the library has support for it, but it does so only for the first
            // input provided to the decoder (basically, start of the stream), and for our
            // use cases, we may get BOM markers in later inputs too (eg: when reading multiple
            // files):
            // https://docs.rs/encoding_rs/0.8.30/encoding_rs/struct.Encoding.html#method.new_decoder_with_bom_removal
            //
            // We can try to maintain separate decoders for each unique stream (eq: by filepath
            // when reading multiple files), but that mandates cleanup of the initialized decoder
            // structs/buffers when they are no longer needed(eg: when files are closed), which
    ...
    pub fn decode_to_utf8(&mut self, input: Bytes) -> Bytes {
        ...
        //
        // We can choose not to strip the BOM marker and keep it as is, but the presence of these
        // extra bytes can throw off any downstream processing we do on the output, and rather
...
pub struct Encoder {
    ...
    // Useful for tracking whether the encoder's encoding is utf-16 (and when it is, its
    // variety). Since encoding_rs does not have encoders for utf-16, this is necessary:
    //
