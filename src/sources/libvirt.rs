    ...
    async fn build(&self, ctx: SourceContext) -> crate::Result<Source> {
        ...
        Ok(Box::pin(async move {
            while let Some(_ts) = ticker.next().await {
                ...
                let result = gather_v2(&turi).await.map_err(|err| {
...
async fn gather_v2(path: &str) -> Result<Vec<Metric>, virt_rpc::Error> {
    let mut cli = virt_rpc::Client::connect(path).await?;
    ...
    for stat in cli.get_all_domain_stats().await? {
        ...
        // Report vcpus
        let vcpus = cli.get_domain_vcpus(&dom, info.nr_virt_cpu as i32).await?;
        for vcpu in vcpus {
