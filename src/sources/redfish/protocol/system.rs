...
mod tests {
    ...
    fn memory() {
        let paths =
            glob::glob("tests/redfish/DSP2043_2024.3/*/Systems/*/Memory/*/index.json").unwrap();

        for path in paths.flatten() {
            let data = std::fs::read_to_string(&path).unwrap();

            match serde_json::from_slice::<Memory>(data.as_bytes()) {
                Ok(_) => {}
                Err(err) => {
                    panic!("{:?}\n{}\n{}", path, err, data);
                }
    ...
    #[test]
    fn system() {
        let paths = glob::glob("tests/redfish/DSP2043_2024.3/*/Systems/*/index.json").unwrap();

        for path in paths.flatten() {
            let data = std::fs::read_to_string(&path).unwrap();

            match serde_json::from_slice::<System>(data.as_bytes()) {
                Ok(_) => {}
                Err(err) => {
                    if err.to_string().contains(r##"missing field `@odata.id`"##) {
                        continue;
                    }
    ...
    fn storage() {
        let paths =
            glob::glob("tests/redfish/DSP2043_2024.3/*/Systems/*/Storage/*/index.json").unwrap();
