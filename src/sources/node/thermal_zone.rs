...
fn load_cooling_device_stats(root: &Path) -> Result<Vec<CoolingDeviceStats>, Error> {
    let pattern = format!(
...
pub async fn collect(paths: Paths) -> Result<Vec<Metric>, Error> {
    ...
    let stats = load_cooling_device_stats(paths.sys())?;
...
mod tests {
    ...
    #[test]
    fn cooling_device_stats() {
        let root = Path::new("tests/node/fixtures/sys");
        let stats = load_cooling_device_stats(root).unwrap();
        assert_eq!(
            stats,
            vec![
                CoolingDeviceStats {
