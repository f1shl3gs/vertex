...
impl TraceContext {
    ...
    pub fn current_with_value<T: 'static + Send + Sync>(value: T) -> Self {
        let mut new_context = TraceContext::current();
        new_context
            .entries
...
pub struct ContextGuard {
    previous_cx: Option<TraceContext>,
...
mod tests {
    ...
    fn nested_contexts() {
        ...
        #[derive(Debug, PartialEq)]
        struct ValueB(u64);
        let _outer_guard = TraceContext::new().with_value(ValueA("a")).attach();
        ...
        {
            let _inner_guard = TraceContext::current_with_value(ValueB(42)).attach();
