...
struct TextItems {
    innodb_version: Option<String>,
    version: Option<String>,
    version_comment: Option<String>,
    wsrep_cluster_name: Option<String>,
    wsrep_provider_options: Option<String>,
    tx_isolation: Option<String>,
    transaction_isolation: Option<String>,
}

pub async fn collect(conn: &mut Connection) -> Result<Vec<Metric>, Error> {
    let mut rows = conn.query(GLOBAL_VARIABLES_QUERY).await?;
    ...
    while let Some(mut row) = rows.next().await? {
        ...
        match name {
            "innodb_version" => text_items.innodb_version = Some(value.to_string()),
            "version" => text_items.version = Some(value.to_string()),
            "version_comment" => text_items.version_comment = Some(value.to_string()),
            "wsrep_cluster_name" => text_items.wsrep_cluster_name = Some(value.to_string()),
            "wsrep_provider_options" => text_items.wsrep_provider_options = Some(value.to_string()),
            "tx_isolation" => text_items.tx_isolation = Some(value.to_string()),
            "transaction_isolation" => text_items.transaction_isolation = Some(value.to_string()),
            _ => {
                if let Some(value) = parse_value(value) {
                    let name = sanitize(name);
                    let desc = get_desc(&name);

                    metrics.push(Metric::gauge(
                        format!("mysql_global_variables_{}", name),
                        desc,
                        value,
                    ));
                }
    ...
    metrics.push(Metric::gauge_with_tags(
        ...
        1,
        tags!(
            "innodb_version" => text_items.innodb_version.unwrap_or_default(),
            "version" => text_items.version.unwrap_or_default(),
            "version_comment" => text_items.version_comment.unwrap_or_default(),
        ),
    ));

    if let Some(value) = text_items.wsrep_cluster_name {
        metrics.push(Metric::gauge_with_tags(
            "mysql_galera_variables_info",
            "PXC/Galera variables information",
            1,
            tags!(
                "wsrep_cluster_name" => value,
            ),
        ))
    }

    if let Some(value) = text_items.wsrep_provider_options {
        metrics.push(Metric::gauge(
    ...
    if let Some(level) = text_items.transaction_isolation.or(text_items.tx_isolation) {
        metrics.push(Metric::gauge_with_tags(
...
fn get_desc(key: &str) -> &'static str {
    const GLOBAL_VARIABLE_DESC: [(&str, &str); 79] = [
    ...
    async fn global_variables() {
        let mut conn = mock(|_query| {
            (vec!["Variable_name", "Value"],
             vec![
                 vec!["wait_timeout", "28800"],
                 vec!["version_compile_os", "Linux"],
                 vec!["userstat", "OFF"],
                 vec!["transaction_prealloc_size", "4096"],
                 vec!["tx_isolation", "REPEATABLE-READ"],
                 vec!["tmp_table_size", "16777216"],
                 vec!["tmpdir", "/tmp"],
                 vec!["sync_binlog", "0"],
                 vec!["sync_frm", "ON"],
                 vec!["slow_launch_time", "2"],
                 vec!["innodb_version", "5.6.30-76.3"],
                 vec!["version", "5.6.30-76.3-56"],
                 vec!["version_comment", "Percona XtraDB Cluster..."],
                 vec!["wsrep_cluster_name", "supercluster"],
                 vec!["wsrep_provider_options", "base_dir = /var/lib/mysql/; base_host = 10.91.142.82; base_port = 4567; cert.log_conflicts = no; debug = no; evs.auto_evict = 0; evs.causal_keepalive_period = PT1S; evs.debug_log_mask = 0x1; evs.delay_margin = PT1S; evs.delayed_keep_period = PT30S; evs.inactive_check_period = PT0.5S; evs.inactive_timeout = PT15S; evs.info_log_mask = 0; evs.install_timeout = PT7.5S; evs.join_retrans_period = PT1S; evs.keepalive_period = PT1S; evs.max_install_timeouts = 3; evs.send_window = 4; evs.stats_report_period = PT1M; evs.suspect_timeout = PT5S; evs.use_aggregate = true; evs.user_send_window = 2; evs.version = 0; evs.view_forget_timeout = P1D; gcache.dir = /var/lib/mysql/; gcache.keep_pages_count = 0; gcache.keep_pages_size = 0; gcache.mem_size = 0; gcache.name = /var/lib/mysql//galera.cache; gcache.page_size = 128M; gcache.size = 128M; gcomm.thread_prio = ; gcs.fc_debug = 0; gcs.fc_factor = 1.0; gcs.fc_limit = 16; gcs.fc_master_slave = no; gcs.max_packet_size = 64500; gcs.max_throttle = 0.25; gcs.recv_q_hard_limit = 9223372036854775807; gcs.recv_q_soft_limit = 0.25; gcs.sync_donor = no; gmcast.listen_addr = tcp://0.0.0.0:4567; gmcast.mcast_addr = ; gmcast.mcast_ttl = 1; gmcast.peer_timeout = PT3S; gmcast.segment = 0; gmcast.time_wait = PT5S; gmcast.version = 0; ist.recv_addr = 10.91.142.82; pc.announce_timeout = PT3S; pc.checksum = false; pc.ignore_quorum = false; pc.ignore_sb = false; pc.linger = PT20S; pc.npvo = false; pc.recovery = true; pc.version = 0; pc.wait_prim = true; pc.wait_prim_timeout = P30S; pc.weight = 1; protonet.backend = asio; protonet.version = 0; repl.causal_read_timeout = PT30S; repl.commit_order = 3; repl.key_format = FLAT8; repl.max_ws_size = 2147483647; repl.proto_max = 7; socket.checksum = 2; socket.recv_buf_size = 212992;"],
             ])
        }).await;
        ...
        assert_contains(
            ...
            vec![
                ...
                (tags!(), 2.0),
                (
                    tags!("innodb_version" => "5.6.30-76.3", "version" => "5.6.30-76.3-56", "version_comment" => "Percona XtraDB Cluster..."),
                    1.0,
                ),
                (tags!("wsrep_cluster_name" => "supercluster"), 1.0),
                (tags!(), 134217728.0),
                (tags!("level" => "REPEATABLE-READ"), 1.0),
            ],
            vec![],
        );
    }
