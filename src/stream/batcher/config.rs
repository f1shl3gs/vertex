...
pub trait BatchConfig<T> {
    ...
    /// Adds a single item to the batch, with the given metadata that was calculated by
    /// `item_fits_in_batch`
    ...
    /// It is safe to assume that `is_batch_full` would return `false` before this is called. You
    /// can return arbitray metadata for an item that will be given back when the item is actually
    /// pushed onto the batch. This is useful if there is an expensive calculation to determine
    /// the "size" of the item.
    fn item_fits_in_batch(&self, item: &T) -> (bool, Self::ItemMetadata);

    /// Returns the maximum amount of time to wait for inputs to a single batch. The timer starts
    /// when the first item is received for a batch.
