//! Trait and related types for Kubernetes HTTP Response

use k8s_openapi::apimachinery::pkg::apis::meta::v1::WatchEvent;
use k8s_openapi::{Resource, ResponseError, WatchResponse};
use serde::de::DeserializeOwned;
use std::fmt::Formatter;

/// The type of errors from parsing an HTTP response as one of the
/// Kubernetes API functions' response types
#[derive(Debug)]
pub enum Error {
    /// An error from deserializing the HTTP response, indicating more data
    /// is needed to complete deserialization
    NeedMoreData,
    ...
    /// An error while deserializing the HTTP response as a string,
    /// indicating that the response data is not UTF-8
...
/// `Response` is used for deserializing Kubernetes API response.
...
{
    fn from_buf(buf: &[u8]) -> Result<(Self, usize), Error> {
        k8s_openapi::Response::try_from_parts(http::StatusCode::OK, buf)
            .map(|item| match item {
