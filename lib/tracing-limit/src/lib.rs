    ...
    S: Subscriber + for<'a> LookupSpan<'a>,
{
    ...
    fn on_event(&self, event: &Event<'_>, ctx: Context<'_, S>) {
        // Visit the event, grabbing the limit status if one is defined. If we
        // can't find a rate limit field, or the rate limit is set as false, then
        // we let it pass through untouched.
        ...
        // that we use to differentiate rate-limited events. This ensures that we won't
        // rate limit an event's _callsite_, but the specific usage of a callsitte, since
        // multiple copies of the same component could be running, etc.
        ...
        // Build the key to represent this event, given its span fields, and see
        // if we're already rate limiting it. If not, we'll initialize an entry
        // for it.
        ...
        // Update our rate limiting state for this event, and see if we should still be
        // rate limiting it.
        //
        // When this is the first time seeing the event, we emit it limke we normally would.
        // The second time we see it in the limit period, we emit a new event to indicate
        // that the original event is being actively rate limited. Otherwise, we don't emit
        // anything.
        ...
        if state.should_limit() {
            ...
            // If we saw this event 3 or more times total, emit an event that indicates
            // the total number of times we rate limited the event in the limit period.
            if previous_count > 1 {
                let message = format!(
            ...
            // We're not rate limiting anymore, so we also emit the current as normal..
            // but we update our rate limiting state since this is effectively equivalent
            // to seeing the event again for the first time.
