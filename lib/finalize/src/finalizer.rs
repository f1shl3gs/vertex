...
use futures::stream::{BoxStream, FuturesOrdered, FuturesUnordered, StreamExt};
    ...
    S: FuturesSet<FinalizerFuture<T>> + Default + Send + Unpin + 'static,
{
    ...
    pub fn new<SD>(shutdown: SD) -> (Self, BoxStream<'static, (BatchStatus, T)>)
    where
        SD: Future + Send + Unpin + 'static,
    {
    ...
    pub fn add(&self, entry: T, receiver: BatchStatusReceiver) {
        if let Some(sender) = &self.sender {
            if let Err(_err) = sender.send((receiver, entry)) {
                // error!(
...
impl<T, S, SD> Stream for FinalizerStream<T, S, SD>
where
