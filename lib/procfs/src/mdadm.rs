...
impl ProcFS {
    pub async fn mdadm(&self) -> Result<Vec<MdStat>, Error> {
        let path = self.root.join("mdstat");
        ...
        for (i, &line) in lines.iter().enumerate() {
            ...
            let (active, total, down, size) =
                eval_status_line(lines[i], lines[i + 1]).map_err(|err| {
                    let msg = format!("parse md device lines failed, err: {:?}", err);
                    Error::invalid_data(msg)
                })?;
...
mod tests {
    ...
    async fn test_parse_mdstat() {
        ...
        let stats = procfs.mdadm().await.unwrap();
