...
impl SysFS {
    pub async fn infiniband(&self) -> Result<Vec<InfiniBandDevice>, Error> {
        let path = self.root.join("class/infiniband");
...
async fn parse_infiniband_device(path: PathBuf) -> Result<InfiniBandDevice, Error> {
    ...
    for sub in ["board_id", "fw_ver", "hca_type"] {
        ...
        let content = match read_to_string(&sp).await {
            ...
            Err(err) => {
                ...
                return Err(Error::Io { path: sp, err });
            }
        };
    ...
    let pp = path.join("ports");
    let mut dirs = tokio::fs::read_dir(&pp)
        .await
        .map_err(|err| Error::Io { path: pp, err })?;
...
async fn parse_infiniband_port(name: &str, root: PathBuf) -> Result<InfiniBandPort, Error> {
    let port_num = root.file_name().unwrap().to_str().unwrap().parse::<u32>()?;
...
fn parse_state(s: &str) -> Result<(u32, String), Error> {
    ...
    if parts.len() != 2 {
        return Err(Error::invalid_data("invalid state for infiniband"));
    }
...
fn parse_rate(s: &str) -> Result<u64, Error> {
    ...
    if parts.len() != 2 {
        return Err(Error::invalid_data("invalid rate for infiniband device"));
    }
                ...
                let content = match read_to_string(path).await {
                    ...
                    Err(err) => {
                        ...
                        return Err(std::io::Error::new(
                            ...
                            format!("failed to read file {}, err: {}", name, err),
                        )
                        .into());
                    }
                };
...
mod tests {
    ...
    async fn test_infiniband_class() {
        ...
        let mut devs = sysfs.infiniband().await.unwrap();
