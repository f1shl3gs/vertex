...
mod test {
    ...
    fn array_remove_from_middle() {
        let mut value = Value::Array(vec![Value::Null, Value::Int64(3)]);
        assert_eq!(value.remove("[0]", false), Some(Value::Null));
        assert_eq!(value.remove("[0]", false), Some(Value::Int64(3)));
    ...
    fn remove_simple() {
        ...
        assert_eq!(value.remove("field", false), Some(Value::Int64(123)));
    ...
    fn remove_prune() {
        ...
        assert_eq!(value.remove("a.d", true), Some(Value::Int64(4)));
        assert_eq!(
            value,
            Value::from(json!({
                "a": {
        ...
        assert_eq!(value.remove("a.b.c[0]", true), Some(Value::Int64(5)));
        assert_eq!(value, Value::from(json!({})));
    }
}
