    ...
    T: Bufferable + Clone,
{
    ...
    {
        usage_handle.set_buffer_limits(Some(self.max_size as usize), None);
        ...
        tokio::spawn(drive_disk_writer(writer, input_rx));
...
async fn drive_disk_writer<T>(mut writer: Writer<T>, mut input: Receiver<T>)
where
    T: Bufferable,
{
    ...
    trace!("disk writer task finished");
}
