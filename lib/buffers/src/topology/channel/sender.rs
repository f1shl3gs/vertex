...
pub enum SenderAdapter<T: Bufferable> {
    ...
    /// The disk buffer.
    Disk(Arc<Mutex<Writer<T, ProductionFilesystem>>>),
}

impl<T: Bufferable> From<LimitedSender<T>> for SenderAdapter<T> {
    fn from(value: LimitedSender<T>) -> Self {
        Self::Memory(value)
    }
}

impl<T: Bufferable> From<Writer<T, ProductionFilesystem>> for SenderAdapter<T> {
    fn from(value: Writer<T, ProductionFilesystem>) -> Self {
        Self::Disk(Arc::new(Mutex::new(value)))
    }
}
    ...
    T: Bufferable,
{
    pub async fn send(&mut self, item: T) -> crate::Result<()> {
        match self {
            Self::Memory(tx) => tx.send(item).await.map_err(Into::into),
            Self::Disk(writer) => {
                let mut writer = writer.lock().await;

                writer.write_record(item).await.map(|_| ()).map_err(|err| {
                    // TODO: Could some errors be handled and not be unrecoverable?
                    // Right now, encoding should theoretically be recoverable -- encoded
                    // value was too big, or error during encoding -- but the traits don't
    ...
    pub(crate) async fn try_send(&mut self, item: T) -> crate::Result<Option<T>> {
        match self {
            ...
            Self::Disk(writer) => {
                ...
                writer.try_write_record(item).await.map_err(|err| {
    ...
    pub fn capacity(&self) -> Option<usize> {
        match self {
            Self::Memory(tx) => Some(tx.available_capacity()),
            Self::Disk(_) => None,
        }
    }
}
    ...
    pub async fn send(&mut self, item: T) -> crate::Result<()> {
        ...
        if let Some(instrumentation) = self.instrumentation.as_ref() {
            if let Some((item_count, item_size)) = item_sizing {
                ...
                if was_dropped {
                    instrumentation.increment_dropped_event_count(
                        item_count as u64,
                        item_size as u64,
                        true,
                    );
