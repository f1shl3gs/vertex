...
impl<T: Bufferable> LimitedSender<T> {
    ...
    /// # Errors
    ///
    /// If the receiver has disconnected (does not exist anymore), then `Err(SendError)`
    ...
    /// `Err(TrySendError::Disconnected)` be returned with the given `item`.
    /// If the channel has insufficient capacity for the item, then
    /// `Err(TrySendError::InsufficientCapacity)` will be returned with the
    /// given `item`.
    pub fn try_send(&mut self, item: T) -> Result<(), TrySendError<T>> {
        // Calculate how many permits we need, and try to acquire them all
        // without waiting.
        let permits_required = self.get_required_permits_for_item(&item);
        let permits = match self
            .inner
...
impl<T> Drop for LimitedSender<T> {
    fn drop(&mut self) {
        // If we're the last sender to drop, close the semaphore on our
        // way out the door.
...
impl<T> Drop for LimitedReceiver<T> {
    fn drop(&mut self) {
        // Notify senders that the channel is now closed by closing the semaphore.
        // Any pending acquisitions will be awoken and notified that the semaphore
        // is closed, and further new sends will immediately see the semaphore is
        // closed.
        self.inner.limiter.close()
    }
}
