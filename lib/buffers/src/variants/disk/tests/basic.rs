    ...
    with_temp_dir(|dir| {
        ...
        async move {
            ...
            let read_task = tokio::spawn(async move {
                ...
                while let Some(mut record) = reader.next().await.expect("reader should not fail") {
...
async fn initial_size_correct_with_multievents() {
    ...
    with_temp_dir(|dir| {
        ...
        async move {
            // Create a regular buffer, no customizations required.
            let (mut writer, _, _) = create_default_buffer(data_dir.clone()).await;
            ...
            let (writer, mut reader, ledger, usage) =
                create_default_buffer_with_usage::<_, MultiEventRecord>(data_dir).await;
            drop(writer);
