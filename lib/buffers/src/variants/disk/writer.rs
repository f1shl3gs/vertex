    ...
    T: Bufferable,
{
    ...
    FailedToEncode(<T as Encodable>::EncodeError),
...
impl<T: Bufferable + PartialEq> PartialEq for WriterError<T> {
    fn eq(&self, other: &Self) -> bool {
        match (self, other) {
            (Self::Io(lerr), Self::Io(rerr)) => lerr.kind() == rerr.kind(),
    ...
    T: Bufferable,
{
    ...
    pub fn archive_record(&mut self, id: u64, record: T) -> Result<WriteToken, WriterError<T>> {
        ...
        let encoded_len = encode_result
            ...
            .map_err(WriterError::FailedToEncode)?;
    ...
    pub async fn flush_record(
        ...
        let flush_result = self.writer.write(event_count, &self.ser_buf[..]).await?;
