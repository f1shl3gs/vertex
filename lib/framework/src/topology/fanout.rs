...
impl Fanout {
    ...
    ///
    /// Will panic if there is no sink with the given ID.
    fn replace(&mut self, id: &ComponentKey, sink: Option<BufferSender<Events>>) {
...
struct Sender {
    #[pin]
    state: SendState<BufferSender<Events>>,
    input: Option<Events>,
    waker: Arc<AtomicWaker>,
}
...
impl Sender {
    fn idle(sink: BufferSender<Events>, waker: Arc<AtomicWaker>) -> Self {
        Self {
    ...
    fn replace(&mut self, sink: BufferSender<Events>) {
