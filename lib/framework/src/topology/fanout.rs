...
impl Fanout {
    ...
    pub fn add(&mut self, id: ComponentKey, sink: BufferSender<Events>) {
        assert!(
    ...
    fn remove(&mut self, id: &ComponentKey) {
        assert!(
            self.senders.swap_remove(id).is_some(),
            "Removing non-existent sink from fanout: {id}"
        );
    }

    /// Replace an existing sink as an output.
    fn replace(&mut self, id: &ComponentKey, sink: BufferSender<Events>) {
        match self.senders.get_mut(id) {
            Some(sender) => {
                // While a sink must be _known_ to be replaced, it must also be empty
                // (previously paused or consumed when the `SendGroup` was created),
                // otherwise an invalid sequence of control operations has been applied.
                assert!(
    ...
    fn pause(&mut self, id: &ComponentKey) {
        match self.senders.get_mut(id) {
            Some(sender) => {
                // A sink must be known and present to be replaced, otherwise an
                // invalid sequence of control operations has been applied.
                assert!(
                    sender.take().is_some(),
    ...
    pub async fn send(&mut self, events: Events) {
        ...
        // replaced
        while let Ok(msg) = self.control_channel.try_recv() {
            self.apply_control_message(msg);
        }
        ...
        // Keep track of whether the control channel has returned `Ready(None)`, and
        // stop polling it once it has. If we don't do this check, it will continue
        // to return `Ready(None)` any time it is polled, which can lead to a busy
        // loop below.
        //
        // In real life this is likely a non-issue, but it can lead to strange behavior
        // in tests if left unhandled.
        let mut control_channel_open = true;

        // Create our send group which arms all senders to send the given events, and
        // handles adding/removing/replacing senders while the send is inflight.
        let mut send_group = SendGroup::new(&mut self.senders, events);

        loop {
            tokio::select! {
                // Semantically, it's not hugely important that this select is biased. It
                // does, however, make testing simpler when you can count on control
                // message being processed first.
                biased;
...
impl<'a> SendGroup<'a> {
    fn new(senders: &'a mut IndexMap<ComponentKey, Option<Sender>>, events: Events) -> Self {
        // If we don't have a valid `Sender` for all sinks, then something went wrong in
        // our logic to ensure we were starting with all valid/idle senders prior to
        // initiating the send.
    ...
    fn add(&mut self, id: ComponentKey, sink: BufferSender<Events>) {
    ...
    fn remove(&mut self, id: &ComponentKey) {
        // we may or may not be removing a sender that we're try to drive a
        // send against, so we have to also detach the send future for the
        ...
        // Now try and detach the inflight send, if it exists.
    ...
    fn replace(&mut self, id: &ComponentKey, sink: Sender) {
        match self.senders.get_mut(id) {
            Some(sender) => {
                // While a sink must be _known_ to be replaced, it must also be
                // empty (previously paused or consumed when the `SendGroup` was
                // created), otherwise an invalid sequence of control operations
                // has been applied.
                assert!(
    ...
    async fn send(&mut self) {
        // Right now, we do a linear scan of all sends, polling each send once in order to
        // avoid waiting forever, such that we can let our control messages get picked up
        // while sends are waiting.
        loop {
            ...
            for (key, send) in &mut self.sends {
                if let Poll::Ready(sender) = poll!(send.get_pin()) {
                    // The send completed, so we restore the sender and mark ourselves
                    // so that this future gets dropped.
            ...
            if !self.sends.is_empty() {
                // We manually yield ourselves because we've polled all of the sends at
                // this point, so if any are left, then we're scheduled for a wake-up...
                // this is a really poor approximation of what `FuturesUnordered` is doing.
                pending!();
            }
        }
    }
...
struct Sender {
    inner: BufferSender<Events>,
    input: Option<Events>,
}

impl Sender {
    fn new(inner: BufferSender<Events>) -> Self {
    ...
    async fn flush(&mut self) {
        if let Some(input) = self.input.take() {
            self.inner
                .send(input)
                .await
                .expect("Sender send event to inner failed");
            self.inner.flush().await.expect("Sender flush inner failed");
        }
    }
}
...
mod tests {
    ...
    async fn build_sender_pairs(
        capacities: &[usize],
    ) -> Vec<(BufferSender<Events>, BufferReceiver<Events>)> {
    ...
    fn _make_events(count: usize) -> impl Iterator<Item = LogRecord> {
        (0..count).map(|i| LogRecord::from(format!("line {}", i)))
    }
    ...
    fn make_event_array(count: usize) -> Events {
