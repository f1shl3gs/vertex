...
pub trait BatchLimiter<T, B> {
    ...
    /// Add a single item to the batch using the metadata that was calculated by
    /// `item_fits_in_batch`
...
pub struct SizeLimit<I> {
    /// The total "size" of all items in a batch. Size is intentionally vague here since
    /// it is user defined, and can vary.
    ///
    ...
    I: ItemBatchSize<T>,
{
    ...
    fn push_item(&mut self, item_size: Self::ItemMetadata) {
