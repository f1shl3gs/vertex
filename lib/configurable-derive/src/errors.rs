...
impl Errors {
    ...
    pub fn duplicate_attrs(
        ...
        self.err_span(
            second.span(),
            &["Duplicate ", attr_kind, " attribute"].concat(),
        );
        self.err_span(
            first.span(),
            &["First ", attr_kind, " attribute here"].concat(),
        );
    }
    ...
    /// Push a `syn::Error` into the list of errors to issue.
...
impl ToTokens for Errors {
    /// Convert the errors into tokens that, when emit, will cause the
    /// user of the macro to receive compiler errors.
