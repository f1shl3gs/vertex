    ...
    loop {
        ...
        *position += used as u64; // do this at exactly same time
        ...
        if done {
            ...
            // We've hit EOF but not yet seen a newline. This can happen when unlucky timing
            // causes us to observe an incomplete write. We write None here and let the loop
            // continue next time the method is called. This is safe because the buffer is
            // specific to this FileWatcher.
            return Ok(None);
        }
    }
}
...
mod tests {
    ...
    fn qc_inner(chunks: Vec<Vec<u8>>, delim: u8, max_size: NonZeroU8) -> TestResult {
        ...
        // block of memory. Where `chunks` simulates what happens when bytes
        // are fitfully available `global_data` is the view of all chunks assembled
        // after every byte is available.
        let mut global_data = BytesMut::new();

        // `DelimDetails` describes the nature of each delimiter found in the `chunks`
        #[derive(Clone)]
        struct DelimDetails {
            ...
            /// Whether this delimiter was within `max_size` of its previous peer
            within_max_size: bool,
            /// Which chunk in `chunks` this delimiter was found in
            chunk_index: usize,
            /// The range of bytes that this delimiter bounds with its previous peer
            byte_range: Range<usize>,
        }

        // Move through the `chunks` and discover at what positions an instance of
        // `delim` exists in the chunk stream and whether that `delim` is more than
        // `max_size` bytes away from the previous `delim`. This loop constructs the
        // `facts` vector that holds `DelimDetails` instances and also populates
        // `global_data`.
        ...
        for (index, chunk) in chunks.iter().enumerate() {
            ...
            match read_until_with_max_size(
                ...
                None => {
                    ...
                    // the max_size range _or_ the current chunk is empty.
                    let has_valid_delimiter = facts
                        .iter()
                        .any(|details| (details.chunk_index == index) && details.within_max_size);

                    assert!(chunk.is_empty() || !has_valid_delimiter)
                }

                Some(total_read) => {
                    // Now that the function has returned we confirm that the returned
                    // details match our `first_delim` and also that the `buffer` is
                    // populated correctly.
    ...
    fn qc_read_until_with_max_size() {
        // The `read_until_with_max` function is intended to be called multiple times
        // until it returns Ok(Some(usize)). The function should never return error in
        // this test. If the return is None we will call until it is not. Once return
        // is Some the interior value should be the position of the first delim in the
        // buffer, unless that delim is past the max_size barrier in which case it will
        // be the position of the first delim that is within some multiple of max_size.
        //
        // I think I will adjust the function to have a richer return type. This will
        // help in the transition to async
        QuickCheck::new()
            .tests(1000)
            .max_tests(2000)
