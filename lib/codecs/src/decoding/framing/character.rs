...
impl CharacterDelimitedDecoder {
    ...
    /// Creates a `CharacterDelimitedDecoder` with a maximum frame length limits.
    ///
...
impl Decoder for CharacterDelimitedDecoder {
    ...
    fn decode(&mut self, buf: &mut BytesMut) -> Result<Option<Self::Item>, Self::Error> {
        loop {
            ...
            // `self.max_length` we discard it, else we return it. At the end
            // of the buffer if the delimiter is not present the remainder
