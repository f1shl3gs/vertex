...
pub fn skip_remaining_coalesce_segments<'a>(
    ...
    loop {
        match path_iter.next() {
            Some(BorrowedSegment::CoalesceField(_field)) => { /* do nothing */ }
...
/// Returns the first matching coalesce key. If none matches, returns
/// Err with the last key.
    ...
    let matched_key = loop {
        match map.get_value(key.as_ref()) {
            ...
            None => {
                ...
                match path_iter.next() {
                    Some(BorrowedSegment::CoalesceField(field)) => key = field,
