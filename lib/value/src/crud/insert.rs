...
use crate::crud::{get_matching_coalesce_key, skip_remaining_coalesce_segments, ValueCollection};
    ...
    match path_iter.next() {
        Some(BorrowedSegment::Field(field)) => {
            if let Some(Value::Object(map)) = value.get_mut_value(key.borrow()) {
                ...
                let prev = insert(&mut map, field.to_string(), path_iter, insert_value);
        ...
        Some(BorrowedSegment::Index(index)) => {
            if let Some(Value::Array(array)) = value.get_mut_value(key.borrow()) {
                ...
                let prev = insert(&mut array, index, path_iter, insert_value);
